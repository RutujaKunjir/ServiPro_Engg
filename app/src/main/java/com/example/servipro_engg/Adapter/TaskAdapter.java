...
import com.example.servipro_engg.Activity.ServiceReminder;
...
public class TaskAdapter extends RecyclerView.Adapter<TaskAdapter.TaskHolder> implements Filterable
{
    ...
    public TaskAdapter(Context mCtx, List<TaskList> vendorLists) {
        this.mCtx = mCtx;
    ...
    public void onBindViewHolder(@NonNull final TaskHolder holder, final int position) {
        ...
        if (status_of_work.equalsIgnoreCase("2") ||
                status_of_work.equalsIgnoreCase("1"))
        {
            holder.optionBtn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view)
                {
                    Intent updateIntent = new Intent(mCtx, AddMachineAmc.class);
        ...
        /*if (status_of_work.equalsIgnoreCase("3") ||
                status_of_work.equalsIgnoreCase("4"))
        {
            holder.optionBtn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view)
                {
                    Intent i1=  new Intent (mCtx, ServiceReminder.class);
                    i1.putExtra(Constants.TASK_DATA,vendorList.getStatus_id());
                    i1.putExtra("sevice_call_no",vendorList.getCall_No());
                    i1.putExtra("customer_id",vendorList.getCust_id());
